...
# Use a temp file because `bin/generate-blockera-site-toolkit-php.php` reads from `blockera-site-toolkit.php`
...
zip -r -q blockera-site-toolkit.zip \
	...
	$(find ./vendor/blockera/site-toolkit/ -type f \( -name "*.php" -o -name "*.json" \)) \
	$(find ./vendor/blockera/telemetry/ -type f \( -name "*.php" -o -name "*.json" \)) \
...
# Reset `blockera-site-toolkit.php`.
git checkout blockera-site-toolkit.php
